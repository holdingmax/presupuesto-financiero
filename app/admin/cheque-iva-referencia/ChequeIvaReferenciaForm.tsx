...
import { subirChequeIvaReferencia } from "./actions";

type EstadoChequeIvaReferencia = {
  total: number;
  duplicadosAmbiguos: number;
  ultimaCarga: string | null;
};

type Props = {
  estadoInicial: EstadoChequeIvaReferencia;
};

export default function ChequeIvaReferenciaForm({ estadoInicial }: Props) {
  ...
  async function handleSubir(e: React.FormEvent) {
    ...
    const resultado = await subirChequeIvaReferencia(formData);
    ...
    setMensajeExito(
      `Se importaron ${resultado.filasImportadas} cheques (${resultado.duplicadosAmbiguos} duplicados ambiguos), reemplazando la carga anterior.`
    );
    setEstado({
      total: resultado.filasImportadas,
      duplicadosAmbiguos: resultado.duplicadosAmbiguos,
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-14">
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-serif font-semibold tracking-tight">Cheques IVA de referencia</h1>
        <p className="mt-2 text-sm text-ink-secondary">
          Planilla de cheques de Macchi (IVA_NUEVO4.xlsx, hoja &quot;operaciones_cheques&quot;),
          usada para distinguir &quot;IVA&quot; de &quot;CH DIFERIDOS IVA&quot; en movimientos
          &quot;CHEQUE P/CAMARA&quot; al subir un extracto bancario.
        </p>
      </div>

      <div className="mb-8 rounded-md border border-line-strong bg-paper-raised px-5 py-4 text-sm">
        <p className="text-ink-secondary">
          Cargados hoy: <span className="font-medium text-ink">{estado.total}</span> cheques, de los
          cuales <span className="font-medium text-ink">{estado.duplicadosAmbiguos}</span> quedaron
          marcados como duplicado ambiguo.
        </p>
      ...
      >
        <p className="text-sm text-ink-secondary mb-1">
          Subí IVA_NUEVO4.xlsx (se lee la hoja &quot;operaciones_cheques&quot;).
        </p>
